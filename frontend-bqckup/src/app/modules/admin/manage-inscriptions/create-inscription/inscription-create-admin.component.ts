...
import {EtatInscriptionAdminService} from '../../../../shared/service/admin/inscriptionref/EtatInscriptionAdmin.service';
  ...
  _skillsFilter:  SkillDto[];
   ...
   private _validInscriptionQuiz = true;
    ...
    displayEtudiant(item: EtudiantDto): string {
        return item && item.fullName ? item.fullName.toString() : "";
    }
	...
	if (value && value.length > 0) {
		this._etudiantsFilter = this.etudiants.filter(s =>
			s.fullName?.toString().toLowerCase()?.includes(value)
		)
	} else {
    ...
    public saveWithShowOption(showList: boolean) {
        this.service.save().subscribe(item => {
            ...
            console.log(error);
            this.alert.show('error', 'An error occurred while saving.');
        });
    }
    ...
    public validateInscriptionNumeroInscription(){
        if (this.item && this.stringUtilService && this.stringUtilService.isEmpty(this.item.numeroInscription)) {
