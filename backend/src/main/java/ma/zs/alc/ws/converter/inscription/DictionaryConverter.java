...
public class DictionaryConverter extends AbstractConverter<Dictionary, DictionaryDto> {
    ...
    public Dictionary toItem(DictionaryDto dto) {
        if (dto == null) {
            ...
            if (StringUtil.isNotEmpty(dto.getWord())) item.setWord(dto.getWord());
            if (StringUtil.isNotEmpty(dto.getDefinition())) item.setDefinition(dto.getDefinition());

            item.setLearned(dto.getLearned());
            if (StringUtil.isNotEmpty(dto.getCreatedOn()))
                item.setCreatedOn(DateUtil.stringEnToDate(dto.getCreatedOn(), DateUtil.DATE_FORMAT_ENG));
    ...
    public DictionaryDto toDto(Dictionary item) {
        if (item == null) {
            ...
            if (StringUtil.isNotEmpty(item.getWord())) dto.setWord(item.getWord());
            if (StringUtil.isNotEmpty(item.getDefinition())) dto.setDefinition(item.getDefinition());
            dto.setLearned(item.getLearned());
            if (StringUtil.isNotEmpty(item.getCreatedOn().toString()))
                dto.setCreatedOn(DateUtil.dateTimeToString(item.getCreatedOn()));
    ...
    public void copy(DictionaryDto dto, Dictionary t) {
        ...
        if (dto.getEtudiant() != null) etudiantConverter.copy(dto.getEtudiant(), t.getEtudiant());
    }
