...
public class SectionCollaboratorServiceImpl extends AbstractServiceImpl<Section, SectionCriteria, SectionDao> implements SectionCollaboratorService {
    ...
    public Section create(Section t) {
        ...
        if (saved != null && t.getQuizs() != null) {
                t.getQuizs().forEach(element-> {
                    element.setSection(saved);
                    quizService.create(element);
            });
        }
    ...
    public Section findWithAssociatedLists(Long id){
        ...
        if(result!=null && result.getId() != null) {
            result.setQuizs(quizService.findBySectionId(id));
    ...
    if(section !=null && section.getId() != null){
            List<List<Quiz>> resultQuizs= quizService.getToBeSavedAndToBeDeleted(quizService.findBySectionId(section.getId()),section.getQuizs());
            quizService.delete(resultQuizs.get(1));
            ListUtil.emptyIfNull(resultQuizs.get(0)).forEach(e -> e.setSection(section));
            quizService.update(resultQuizs.get(0),true);
