...
import ma.zs.alc.service.impl.util.InscriptionCollaboratorStateConstant;
...
import static ma.zs.alc.service.impl.util.InscriptionCollaboratorStateConstant.*;
...
public class InscriptionCollaboratorCollaboratorServiceImpl extends AbstractServiceImpl<InscriptionCollaborator, InscriptionCollaboratorCriteria, InscriptionCollaboratorDao> implements InscriptionCollaboratorCollaboratorService {
    ...
    public void updatePaidAndFreeWhereEndDateIsGreaterThanNow() {
        ...
        for (InscriptionCollaborator inscription : inscriptions) {
            LocalDateTime now = LocalDateTime.now();
            updateInscription(inscription, pendingState,now);
        }
    }

    private void updateInscription(InscriptionCollaborator inscription, InscriptionCollaboratorState pendingState,LocalDateTime now) {
        if (inscription != null && inscription.getInscriptionCollaboratorState() != null) {
            String stateLibelle = inscription.getInscriptionCollaboratorState().getLibelle();
            if (inscription.getEndDate() != null && (FREE.equals(stateLibelle) || PAID.equals(stateLibelle)) && inscription.getEndDate().isBefore(now)) {
                inscription.setInscriptionCollaboratorState(pendingState);
                dao.save(inscription);
            }
        }
    }
